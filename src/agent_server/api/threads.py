...
async def get_thread_state(
    ...
    Returns the latest state snapshot including todos, files, and messages.
    """
    logger = logging.getLogger(__name__)
    ...
    # Get the latest state from history (limit=1)
    req = ThreadHistoryRequest(
        limit=1,
        before=None,
        metadata=None,
    ...
    states = await get_thread_history_post(thread_id, req, user, session)

    if not states:
        # Return empty state if no history
        return ThreadState(
            values={},
            next=[],
            tasks=[],
            interrupts=[],
            metadata={},
            checkpoint=ThreadCheckpoint(checkpoint_id="", checkpoint_ns=""),
        )
